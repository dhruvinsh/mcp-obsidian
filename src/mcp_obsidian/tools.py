...
obsidian_port = int(os.getenv("OBSIDIAN_PORT", 27124)) # Read OBSIDIAN_PORT
obsidian_protocol = os.getenv("OBSIDIAN_PROTOCOL", "http") # Read OBSIDIAN_PROTOCOL
...
class DeleteFileToolHandler(ToolHandler):
   ...
   def run_tool(self, args: dict) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
       if "filepath" not in args:
           raise RuntimeError("filepath argument missing in arguments") # Added missing raise
       
       if not args.get("confirm", False):
           raise RuntimeError("Deletion not confirmed. Set 'confirm' to true.") # Added missing raise
